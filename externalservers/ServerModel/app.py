...
def do_train_request(url, the_json):
...
def predict():
	...
	# Create 'tempt' dir if it does not exist
	...
	proc_img = ai.preprocess_image("./temp/" + filename)
	...
	if 1 in models:
		...
		# Prediction results
		results_cm = ai.conv_model_predict(proc_img, signer, sclient)
		dict_cm = {"prediction": results_cm[0], "signerp": results_cm[1]}
		if results_cm[1] == 'correct':
			dict_cm['confidence'] = str(round(results_cm[2]*100, 2))
		else:
			pass
	...
	if 2 in models:
		...
		# Prediction results
		results_ssm = ai.signer_signature_model_predict(proc_img, signer, sclient)
		dict_ssm = {"prediction": results_ssm[0], "signerp": results_ssm[1]}
		if results_ssm[1] == 'correct':
			dict_ssm['confidence'] = str(round(results_ssm[2]*100, 2))
		else:
			pass
	...
	if 3 in models:
		...
		# Prediction results
		results_sm = ai.siamese_model_predict(proc_img, signer, sclient, worker())

		dict_sm = {}

		# Check errors
		if results_sm[0] == 0:
			dict_sm['success'] = False
		else:
			dict_sm['success'] = True
			dict_sm['prediction'] = results_sm[1]
...
def train():
	...
	if len(server_url) > 0:
		thread = Process(target=do_train_request, args=[server_url + 'train', train_json])
		thread.start()
	return jsonify(server_response)
...
def info():
	...
	if typem == 1:
		...
		try:
			...
			server_response['lastdate'] = cdate[1] + " " + cdate[2] + " " + cdate[-1] + " at " + cdate[3]
		...
		model_path = "./Models/SiameseModel" + ("/client" if sclient else "/employee") 
		try:
			server_response['exists'] = True

			cdate = str(time.ctime(os.path.getctime(model_path + "/siamese_model.h5"))).split(' ')
			server_response['lastdate'] = cdate[1] + " " + cdate[2] + " " + cdate[-1] + " at " + cdate[3]
