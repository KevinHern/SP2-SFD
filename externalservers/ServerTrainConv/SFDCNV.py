...
def train_conv(dir_path, signers, sclient):
	...
	model = create_arch1(inputShape, 2*len(signers), 'sigmoid')
	...
	try:
		...
		server_response = requests.post(modelserver_url + 'store',
				json = {'typem': 1, "file": list(filem), "filename": filename, "classes": signers, "sclient": sclient}).json()
	except:
		pass
	finally:
